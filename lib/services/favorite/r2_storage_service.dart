...
class R2StorageService {
  ...
  Future<bool> deleteSongFiles(String songId) async {
    ...
    try {
      ...
      // 删除音频文件
      if (!await _deleteFilesWithPrefix(audioPrefix)) {
        Logger.warning('R2 音频文件删除失败: $songId', 'R2Storage');
        allSuccess = false;
      }
      // 删除封面文件
      if (!await _deleteFilesWithPrefix(coverPrefix)) {
        Logger.warning('R2 封面文件删除失败: $songId', 'R2Storage');
        allSuccess = false;
      }
  ...
  /// 删除指定前缀的所有文件，返回是否全部成功
  Future<bool> _deleteFilesWithPrefix(String prefix) async {
    ...
    try {
        ...
        for (final item in obj.objects) {
          if (item.key != null) {
            try {
              ...
              Logger.error('删除R2对象失败: ${item.key}', e, null, 'R2Storage');
              allSuccess = false;
            }
          }
        }
      ...
      Logger.error('列出/删除前缀文件失败: $prefix', e, null, 'R2Storage');
      return false;
    }
    return allSuccess;
  }
  ...
  /// 生成预签名 URL（临时访问链接）
  /// [objectName] 对象名称
  /// [expirySeconds] 过期时间（秒），默认1小时
  Future<String?> getPresignedUrl(String objectName, {int expirySeconds = 3600}) async {
    ...
    try {
      ...
      Logger.success('生成预签名URL: ${url.length > 50 ? '${url.substring(0, 50)}...' : url}', 'R2Storage');
      return url;
    } catch (e) {
