...
class MobileAudioPlayer implements AudioPlayerInterface {
  ...
  Future<void> _initialize() async {
    ...
    _subscriptions.add(_player!.playbackEventStream.listen(
      (event) {
        Logger.debug('【事件】playbackEvent - state: ${event.processingState}', 'MobileAudioPlayer');
      },
    ...
    try {
      ...
      try {
        if (song.audioUrl.startsWith('http')) {
          ...
          Logger.info('【播放】已停止当前播放', 'MobileAudioPlayer');
          // 设置新URL
          duration = await _safePlayer.setUrl(song.audioUrl);
        } else if (song.audioUrl.startsWith('file://')) {
          Logger.info('【播放】使用 setFilePath 加载本地文件', 'MobileAudioPlayer');
          await _safePlayer.stop();
          duration = await _safePlayer.setFilePath(song.audioUrl.substring(7));
        } else {
