...
class MusicApiService {
  ...
  int? _parseDuration(String? interval) {
    ...
    try {
      ...
      Logger.debug('解析时长失败: $interval', 'MusicApiService');
      return null;
    }
  }
  ...
  Future<String?> getLyrics({
    ...
    final result = await getLyricsWithTranslation(songId: songId, songMid: songMid);
    return result?['lrc'];
  }
  ...
  Future<Map<String, dynamic>> getPlaylistSongs({
    ...
    try {
      ...
      Logger.debug('请求歌单API: $playlistId, 页码: $page, 数量: $num', 'MusicApiService');
  ...
  Future<String?> getSongUrl({
    ...
    final qualityCode = quality ?? AudioQualityService.instance.getCurrentQualityCode();
    Logger.debug('获取歌曲URL，使用音质代码: $qualityCode', 'MusicApiService');
    try {
      final queryParams = {'quality': qualityCode.toString()};
      ...
      Logger.error('获取歌曲URL失败', e, null, 'MusicApiService');
      return null;
    }
  }
}
